...
from constants import (
    ASRConfig, DiarizationConfig, PreciseRerunConfig,
...
def main():
    ...
    if VOCAL_AUDIO != str(audio):
        print(f"[preproc] {mode} → {VOCAL_AUDIO}")
    ...
    if PREC.enabled:
        ...
        if spans:
            ...
            print(f"[precise] {len(spans)} spans (~{total:.1f}s) → {PREC.model} beam={PREC.beam_size} patience={PREC.patience}")
            ...
            print("[precise] no hard spans detected; skipping")
    ...
    if sp_rules:
        ...
        print(f"[post] spelling map applied ({len(sp_rules)} rules)")
